import{H as m,j as P,t as N,x as T}from"./app-DSlg_KVY.js";let A=!1;function O(){A||(m.registerLanguage("javascript",P),m.registerLanguage("typescript",N),m.registerLanguage("xml",T),m.configure({ignoreUnescapedHTML:!0}),A=!0)}const U={react:"javascript",ts:"typescript",js:"javascript",html:"xml"};function H(e){O(),delete e.dataset.highlighted;try{m.highlightElement(e)}catch{}}const x="18.3.1",k="https://esm.sh",I="https://cdn.jsdelivr.net/npm/@babel/standalone@7.26.4/babel.min.js";function B(e){const t=String(e||"").split(/\r?\n/);let o="react",i="",l=300;if(/^\s*(#!|%%)/.test(t[0]||"")){const c=t[0].replace(/^\s*(#!|%%)\s*/,"");t.shift();const b=c.match(/\b(react|ts|js|html)\b/);b&&(o=b[1]);const f=c.match(/title=("([^"]*)"|(\S+))/);f&&(i=(f[2]??f[3]??"").trim());const p=c.match(/height=(\d+)/);p&&(l=Math.max(140,Math.min(720,Number(p[1]))))}return{kind:o,title:i||{react:"App.jsx",ts:"App.tsx",js:"main.js",html:"index.html"}[o],height:l,code:t.join(`
