...
`401`은 로그인 화면으로 보내야 할 수 있고, `403`은 권한이 없다는 메시지를 보여줘야 한다. `404`는 없는 리소스이고, `500`은 사용자가 해결할 수 없는 서버 문제다.

### REST를 오해하는 지점

REST는 URL을 예쁘게 짓는 규칙이 아니다. 리소스를 중심으로 HTTP의 의미를 활용하는 설계 스타일이다. `/createPost`보다 `POST /posts`가 더 REST다운 이유가 여기에 있다.
