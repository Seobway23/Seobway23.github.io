...
readTime: 13
featured: false
coverImage: /roadmap-thumbnails/step-07-db-orm.svg
...
테이블을 만들 때는 `id`만 보지 말고 unique, not null, foreign key, index를 함께 본다. 어떤 값이 중복되면 안 되는지, 어떤 관계가 끊기면 안 되는지, 어떤 조회가 자주 일어나는지를 스키마에 반영해야 한다.
