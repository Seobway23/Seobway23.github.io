...
> 용어를 모르는 분도 읽을 수 있도록, 등장하는 단어마다 바로 설명을 덧붙였습니다.
...
본격적으로 시작하기 전에, 등장하는 단어들을 한 번에 정리합니다.

### 1-1. 핵심 용어 정리

**Docker (도커)**
- 앱과 그 실행 환경(OS, 라이브러리 등)을 **컨테이너**라는 박스에 묶어서 어디서든 똑같이 실행되게 해주는 도구
- **이미지(Image)**: 컨테이너의 설계도 (레시피)
- **컨테이너(Container)**: 이미지를 실제로 실행한 것 (요리된 음식)
- **레지스트리(Registry)**: 이미지를 저장·공유하는 창고 (Docker Hub, AWS ECR 등)
- **이미지 태그(Tag)**: 이미지의 버전 식별자 (예: `app:v1.0`, `app:abc1d2e`)

**Kubernetes (쿠버네티스, K8s)**
- 여러 서버(노드)에 걸쳐 수십~수백 개의 컨테이너를 자동으로 배포·관리·복구해주는 시스템
- K8s는 Kubernetes의 약자 (K와 s 사이에 8글자)

**Pod (파드)**
- K8s에서 컨테이너를 실행하는 가장 작은 단위
- 하나의 Pod에 하나 또는 여러 컨테이너가 들어감
- Pod이 죽으면 K8s가 자동으로 새 Pod을 만들어줌

**Deployment (디플로이먼트)**
- K8s에서 "이 앱을 몇 개 실행할지, 어떻게 업데이트할지"를 정의하는 리소스
- `replicas: 3`이면 Pod이 항상 3개 유지됨 (하나가 죽으면 자동 재생성)

**StatefulSet (스테이트풀셋)**
- Deployment와 비슷하지만 **고정된 이름과 저장소**가 필요한 앱(MySQL, Redis 등)에 사용
- Pod 이름이 `mysql-0`, `mysql-1`처럼 고정됨 → 데이터베이스 복제 구성 시 필수

**Service (서비스)**
- K8s에서 Pod들에게 하나의 고정 주소(IP)를 붙여주는 리소스
- Pod이 재생성되어 IP가 바뀌어도, Service 주소는 그대로 → 앱끼리 통신 가능

**Namespace (네임스페이스)**
- K8s 클러스터 안에서 리소스를 논리적으로 분리하는 공간
- 예: `databases` 네임스페이스(MySQL, Redis), `app-prod` 네임스페이스(웹 앱)

**PVC / PV (영구 볼륨)**
- **PV(PersistentVolume)**: 실제 저장 공간 (SSD, 클라우드 디스크 등)
- **PVC(PersistentVolumeClaim)**: Pod이 "나 저장 공간 10GB 필요해"라고 요청하는 것
- Pod이 죽어도 PVC에 묶인 데이터는 사라지지 않음

**Jenkins (젠킨스)**
- 코드를 빌드·테스트·배포하는 과정을 자동화해주는 CI(지속적 통합) 서버
- 개발자가 코드를 push하면 Jenkins가 자동으로 파이프라인을 실행

**CI/CD**
- **CI (Continuous Integration, 지속적 통합)**: 코드 변경이 생길 때마다 자동으로 빌드·테스트
- **CD (Continuous Deployment/Delivery, 지속적 배포)**: 빌드된 결과를 자동으로 서버에 배포
- Jenkins는 CI, ArgoCD는 CD 역할

**ArgoCD (아르고CD)**
- Git 저장소를 보고 있다가, 변경이 생기면 K8s 클러스터에 자동으로 배포해주는 CD 도구
- GitOps 방식의 CD 도구

**GitOps (깃옵스)**
- "Git 저장소가 인프라의 유일한 진실"이라는 방식
- 서버 상태를 직접 바꾸는 게 아니라, Git에 원하는 상태를 commit하면 ArgoCD가 알아서 맞춰줌
- 모든 배포 이력이 Git history에 남아 추적 가능

**Kustomize (커스터마이즈)**
- K8s 배포 파일(YAML)을 환경별로 쉽게 관리하는 도구
- 기본 설정(base)과 환경별 덮어쓰기(overlay)를 분리해서 관리

**Commit SHA (커밋 해시)**
- Git이 각 commit에 자동으로 붙이는 고유 식별자 (예: `abc1d2e`)
- 이미지 태그에 SHA를 붙이면 "어느 코드 버전으로 만든 이미지인지" 바로 추적 가능

**kubeconfig**
- K8s 클러스터에 접속하기 위한 인증 정보가 담긴 파일 (`~/.kube/config`)
- 클러스터 주소, 인증서, 토큰 등이 포함됨

**base64**
- 바이너리 데이터(인증서, 키 등)를 텍스트로 안전하게 변환하는 인코딩 방식
- Jenkins Credentials에 kubeconfig를 저장할 때 base64로 인코딩해서 전달
...
| **네트워크 주소** | Pod이 재시작되면 IP가 바뀜 | `mysql-0.mysql.ns.svc` 고정 DNS |
...
Deployment라면 Pod이 재시작될 때마다 IP가 바뀌어 이 설정이 깨집니다. StatefulSet은 `mysql-0`이라는 이름(DNS)이 고정되므로 안전합니다.
...
spec:
  ...
  # volumeClaimTemplates: Pod이 생성될 때 자동으로 PVC(디스크)도 함께 생성
...
이 과정에서 서비스가 중단되지 않으려면, **항상 Ready 상태의 Pod이 요청을 처리할 수 있어야** 합니다.
  ...
  template:
    metadata:
      labels:
        app: myapp
    spec:
      # SIGTERM 신호 후 Pod이 강제 종료되기까지 대기 시간
