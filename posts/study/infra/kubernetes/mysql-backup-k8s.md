...
기존 스크립트는 `/home/vegeterian/backup/container/rsync_backup.sh`였다. 노드 crontab에 등록되어 있었고, 새벽 3시에 자동 실행됐다.

```bash
0 3 * * * /home/vegeterian/backup/container/rsync_backup.sh
```
...
스크립트 주석에도 "MySQL (컨테이너 중지 후 권장)"이라고 적혀 있었다. 알고 있었지만 중지하지 않고 돌리고 있었다는 뜻이다. 이 상태의 덤프로 복구하면 InnoDB 내부 불일치로 복구 자체가 실패할 수 있다.[^mysql-backup-types]

### 문제 2: 실패가 가려지는 구조

대상 경로(`/mnt/backup/daesan-server/...`)가 존재하지 않으면 rsync가 `mkdir ... failed: No such file or directory` 오류를 냈다. 그런데 스크립트는 오류 여부를 확인하지 않고 마지막에 무조건 "백업 완료"를 로그에 남겼다.

```bash
# 실제 스크립트의 구조 (의사 코드)
rsync -av /source/ /mnt/backup/daesan-server/  # 실패해도
echo "백업 완료 $(date)"                          # 항상 출력
```

새벽 3시 로그를 아침에 확인하면 항상 성공처럼 보였다.

### 문제 3: PMS DB를 백업하는 건지 불명확

`pms-web` 네임스페이스에는 MySQL Pod가 없었다. MySQL은 다른 스택(`daesan-webpage-server`)에서 운영됐고, 스크립트는 그 스택의 볼륨 경로를 백업했다. 과연 이 백업이 PMS 운영 DB를 백업하는 것인지 확인할 방법이 없었다.
...
DB 비밀번호를 스크립트에 하드코딩하지 않는다. 이미 `pms-web-secrets`라는 Secret이 있고, 거기에 `DATABASE_URL`이 들어 있었다.
...
```bash
sudo mkdir -p /data/pms/mysql-dumps
sudo chmod 700 /data/pms/mysql-dumps
```
        ...
        spec:
          ...
          containers:
            - name: mysqldump
              ...
              envFrom:
                - secretRef:
                    name: pms-web-secrets   # DATABASE_URL 주입
              volumeMounts:
                - name: backup-volume
                  mountPath: /backup
              command:
...
`--single-transaction`이 핵심이다. InnoDB 테이블에서 `BEGIN TRANSACTION` → 덤프 → `COMMIT` 순으로 동작해, 다른 트랜잭션이 커밋되더라도 일관된 시점의 데이터를 읽는다.[^mysqldump-docs] 서비스를 중단하지 않아도 된다.
...
```bash
kubectl apply -f /root/mysql-dump-cronjob.yaml -n pms-web
kubectl get cronjob -n pms-web mysql-dump
```
...
```bash
# 수동 실행
kubectl -n pms-web delete job mysql-dump-manual --ignore-not-found
kubectl -n pms-web create job --from=cronjob/mysql-dump mysql-dump-manual

# 완료 대기
kubectl -n pms-web wait \
  --for=condition=complete job/mysql-dump-manual \
  --timeout=600s

# 로그 확인
kubectl -n pms-web logs job/mysql-dump-manual --tail=200
```

로그에서 아래를 확인한다:

```
[2026-04-06 03:00:12] 백업 시작: pms_db → /backup/pms_db_20260406_030012.sql.gz
[2026-04-06 03:00:45] 백업 완료: 48M /backup/pms_db_20260406_030012.sql.gz
...
```bash
# 노드에서
ls -la /data/pms/mysql-dumps | tail
du -sh /data/pms/mysql-dumps

# 파일 내용 앞부분 (mysqldump 헤더와 버전 정보)
zcat /data/pms/mysql-dumps/pms_db_20260406_030012.sql.gz | head -n 40

# CREATE TABLE 목록
zcat /data/pms/mysql-dumps/pms_db_20260406_030012.sql.gz | grep -n "CREATE TABLE"
```
...
다음 글에서는 이 덤프 파일을 실제로 K3s 클러스터 안에 임시 MySQL Pod를 띄워 import하고, 테이블이 정상인지 확인하는 과정을 다룬다.

---

## References

[^mysql-backup-types]: MySQL Documentation — Backup and Recovery Types: https://dev.mysql.com/doc/refman/8.0/en/backup-types.html
[^mysqldump-docs]: MySQL Documentation — mysqldump: https://dev.mysql.com/doc/refman/8.0/en/mysqldump.html
[^k8s-cronjob]: Kubernetes Documentation — CronJob: https://kubernetes.io/docs/concepts/workloads/controllers/cron-jobs/
[^innodb-backup]: MySQL Documentation — InnoDB Backup: https://dev.mysql.com/doc/refman/8.0/en/innodb-backup.html
