...
- [ISP — 인터넷 접속을 파는 사람들 →](/post/isp-internet-service-provider) — IXP에서 만나는 사업자들의 계층 구조
- [CDN — 콘텐츠를 가장 빠른 경로로 전달하는 방법 →](/post/cdn-content-delivery-network) — IXP 피어링을 활용해 속도를 높이는 콘텐츠 네트워크
- [회선 교환 vs 패킷 교환 →](/post/circuit-vs-packet-switching) — IXP가 주고받는 데이터의 전달 방식
