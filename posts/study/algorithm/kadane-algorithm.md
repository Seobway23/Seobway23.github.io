...
- Python 구현도 동일하게 “현재까지 최적(현재 인덱스에서 끝나는 최적)”과 “전체 최적” 두 변수를 갱신한다.
...
## 구현 (TypeScript) — 구간 인덱스까지 구하기

실전에서는 합만이 아니라 “어떤 구간이냐”가 필요할 때가 많다.

```typescript
export function maxSubarray(nums: number[]): { sum: number; l: number; r: number } {
  if (nums.length === 0) throw new Error("nums is empty");
...
def max_subarray(nums: List[int]) -> Tuple[int, int, int]:
    """
    Returns (best_sum, l, r), where nums[l:r+1] is the maximum-sum subarray.
    """
    if not nums:
        raise ValueError("nums is empty")
