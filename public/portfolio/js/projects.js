...
window.addEventListener("pause-3d", () => pause3D());
window.addEventListener("resume-3d", () => resume3D());
document.addEventListener("visibilitychange", () => {
  if (document.hidden) pause3D();
  else resume3D();
});
...
function onPointerMove(e) {
  // Pointermove는 빈도가 매우 높아서, rAF 1회로 스로틀 + Raycaster 재사용
  lastPointerEvent = e;
  if (pendingPointerCheck) return;
  pendingPointerCheck = true;
  requestAnimationFrame(() => {
    pendingPointerCheck = false;
    if (!lastPointerEvent || paused) return;
    mouse.set(
      (lastPointerEvent.clientX / window.innerWidth) * 2 - 1,
      -(lastPointerEvent.clientY / window.innerHeight) * 2 + 1,
    );
...
function createUnderGlowTexture() {
  ...
  // Softer, wider bloom (underglow spread)
...
function createBoardGlowTexture() {
  ...
  // soft edge vignette to keep glow near keys
...
function createLegend(
  ...
  // OFF 모드(가산 블렌딩)에서 글자 '지지직' 방지: 밉맵/필터 고정
...
function createKeyboard() {
  ...
  // V15 Unified Bezel (Integrated Frame)
  ...
  // Board glow plane (facing up toward keys) — used in OFF mode
  ...
  // 큰 직사각형(보드 윗면 전체가 은은히 빛나는 느낌)
  // frame outer size ≈ fullScreenElement + 0.6, 약간 안쪽으로 줄여서 가장자리 깨짐 방지
  ...
  // 키 바닥(-0.1 근처) 아래, 프레임 윗면(≈0.275)보다 아래로 두어 자연스럽게 보이게
  ...
  layout.forEach((c) => {
    ...
    // Per-key backlights (distributed under-key LEDs)
    const makeBL = () => {
    ...
    // Place slightly below keycap, spread toward corners
    ...
    // Per-key underglow "bloom" layer (fake light spread)
    ...
    // Secondary wider glow for extra spread (very subtle)
...
function applyLightMode(mode) {
  ...
  // OFF: 외부 조명은 거의 끄고, 내부 백라이트 + 레전드 발광을 살림
  ...
  keys.forEach((kG) => {
    ...
    const backLights = kG.userData?.backLights;
    if (backLights) {
      // Stronger "under-key" backlight in OFF mode
    ...
    if (underGlow) {
      ...
      // keep it subtle; acts like spread/bloom around the key base
    ...
    const accent = kG.userData?.accentLight;
    // Keep a subtle blue accent even in OFF mode (aurora keys)
    if (accent) {
